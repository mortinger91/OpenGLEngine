...
Material::Material(const std::string& texturePath, float _m_Shininess, float _m_Specular, float _m_Diffuse, float _m_Ambient, const std::string& shaderPathVert, const std::string& shaderPathFrag)
	: m_Shininess(_m_Shininess),
	  m_Specular(_m_Specular),
	  m_Diffuse(_m_Diffuse),
	  m_Ambient(_m_Ambient),
	  m_Texture(std::make_unique<Texture>(texturePath)),
	  m_Shader(std::make_unique<Shader>(shaderPathVert, shaderPathFrag))
{
